...
const handleOutput = (type: string, data: any) => {
  ...
  if (type === "PROGRESS") {
    ...
    if (data.unfollowers.length === 0) {
      ...
      data.unfollowers.forEach((unfollower: { username: string; isVerified: boolean }) =>
...
class Script {
  private checkVerifiedUsers: boolean;
  private unfollowers: Array<{ username: string; isVerified: boolean }>;
  private canQuery: boolean;
  private nextPageHash: string;
  private requestsCount: number;
  private followingCount: number;
  private currentPageCount: number;
  private estimatedStepValue: number;

  constructor(checkVerifiedUsers: boolean) {
  ...
  createURLParamsString(params: { [key: string]: string }) {
  ...
  async startScript() {
    try {
      while (this.canQuery) {
        ...
        data.user.edge_follow.edges.forEach((edge: { node: { follows_viewer: boolean; is_verified: boolean; username: string } }) => {
